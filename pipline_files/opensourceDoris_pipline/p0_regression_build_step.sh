...
case_center=${cluster_center}/clusterRegressionCenter/P0/
work_path=${cluster_center}/clusterConf/P0/
...
#check external docker
if [[ -d %teamcity.build.checkoutDir%/docker ]]; then
    rm -rf ${case_center}/${cluster_name}/docker
...
if [ "_$?" != "_0" ]; then
    ...
    Backup_cluster_name=${pullrequestID}_%build.vcs.number%_p0
...
echo "we got files:"
ls -l %teamcity.build.checkoutDir%/
echo
if [[ "_%run_external_case%" == "_true" && -d ${case_center}/${cluster_name}//docker ]]; then
    ...
    if [ ! -d /data/regression/load/tpch1_parquet/tpch1.db.tar.gz ]; then
    ...
    #cd ${case_center}/${cluster_name}/docker/thirdparties && bash stop-thirdparties-docker.sh
    sleep 5
    #启动外部组件的docker
    cd ${case_center}/${cluster_name}/docker/thirdparties && bash start-thirdparties-docker.sh
    #执行回归
    cd ${case_center}/${cluster_name}
    echo "RUN REGRESSION CASE"
