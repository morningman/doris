...
public class PushdownFilterThroughWindow extends OneRewriteRuleFactory {
    ...
    public Rule build() {
        return logicalFilter(logicalWindow()).thenApply(ctx -> {
            ...
            Set<Expression> commonPartitionKeys = window.getCommonPartitionKeyFromWindowExpressions();
            ...
            LogicalFilter bottomFilter = new LogicalFilter(bottomConjuncts, window.child());
            window = (LogicalWindow<Plan>) window.withChildren(bottomFilter);
            LogicalFilter upperFilter = (LogicalFilter) filter.withConjuncts(upperConjuncts).withChildren(window);
            return upperFilter;
