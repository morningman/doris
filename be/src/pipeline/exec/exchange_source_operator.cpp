...
Status ExchangeSourceOperator::init(const ExecNode* exec_node, RuntimeState* state) {
    ...
    _exchange_node = dynamic_cast<doris::vectorized::VExchangeNode *>(const_cast<ExecNode*>(exec_node));
    return Status::OK();
}
...
Status ExchangeSourceOperator::prepare(RuntimeState* state) {
    ...
    _exchange_node->_stream_recvr = state->exec_env()->vstream_mgr()->create_recvr(
            state, _exchange_node->_input_row_desc, state->fragment_instance_id(), _exchange_node->_id, _exchange_node->_num_senders,
            config::exchg_node_buffer_size_bytes, _exchange_node->_runtime_profile.get(), _exchange_node->_is_merging,
            _exchange_node->_sub_plan_query_statistics_recvr);
    _exchange_node->_rows_returned_counter = ADD_COUNTER(_runtime_profile, "RowsReturned", TUnit::UNIT);
