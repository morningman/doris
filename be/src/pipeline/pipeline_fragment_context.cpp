...
PipelineFragmentContext::PipelineFragmentContext(const TUniqueId &query_id, const TUniqueId &instance_id,
                                                 std::shared_ptr<QueryFragmentsCtx> query_ctx, ExecEnv *exec_env) :
                                                 _query_id(query_id), _fragment_instance_id(instance_id),
                                                 _exec_env(exec_env),
                                                 _cancel_reason(PPlanFragmentCancelReason::INTERNAL_ERROR),
                                                 _closed_pipeline_cnt(0),_query_ctx(std::move(query_ctx)) {}

PipelineFragmentContext::~PipelineFragmentContext() {}

void PipelineFragmentContext::cancel(const PPlanFragmentCancelReason& reason, const std::string& msg) {
    if (!_cancelled) {
...
Status PipelineFragmentContext::prepare(const doris::TExecPlanFragmentParams& request) {
    ...
    // 基于量量化的算子做POC
    if (!request.query_options.__isset.enable_vectorized_engine || !request.query_options.enable_vectorized_engine) {
        return Status::InternalError("should set enable_vectorized_engine to true");
    }

    // 1. 创建初始化RuntimeState
    _runtime_state = std::make_unique<RuntimeState>(params, request.query_options, _query_ctx->query_globals, _exec_env);
    ...
    RETURN_IF_ERROR(ExecNode::create_tree(_runtime_state.get(), _runtime_state->obj_pool(), request.fragment.plan,
                                          *desc_tbl, &_root_plan));
...
//            || typeid(*node) == typeid(vectorized::NewJdbcScanNode)
    ...
    if (request.fragment.__isset.output_sink) {
        RETURN_IF_ERROR(DataSink::create_data_sink(
                _runtime_state->obj_pool(), request.fragment.output_sink, request.fragment.output_exprs, params,
...
//        }
    ...
    for (PipelinePtr& pipeline : _pipelines) {
        ...
        _tasks.emplace_back(std::make_unique<PipelineTask>(pipeline, 0, _runtime_state.get(), operators, sink, this));
    }
...
Status PipelineFragmentContext::_create_sink(const TDataSink& thrift_sink) {
    ...
    switch (thrift_sink.type) {
        case TDataSinkType::DATA_STREAM_SINK: {
            ...
            sink_ = std::make_shared<ExchangeSinkOperatorTemplate>(next_operator_template_id(), "", nullptr, exchange_sink);
            break;
        }
        case TDataSinkType::RESULT_SINK: {
            auto* result_sink = dynamic_cast<doris::vectorized::VResultSink*>(_sink.get());
            sink_ = std::make_shared<ResultSinkOperatorTemplate>(next_operator_template_id(), "", nullptr, result_sink);
            break;
        }
        default:
            return Status::InternalError("Unsuported sink type in pipeline: {}", thrift_sink.type);
