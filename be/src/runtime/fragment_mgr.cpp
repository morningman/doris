...
Status FragmentMgr::exec_plan_fragment(const TExecPlanFragmentParams& params) {
    if (params.txn_conf.need_txn) {
        ...
        if (params.query_options.__isset.enable_vectorized_engine && params.query_options.enable_vectorized_engine) {
...
Status FragmentMgr::exec_pipeline(const TExecPlanFragmentParams& params) {
    ...
    std::shared_ptr<pipeline::PipelineFragmentContext> context = std::make_shared<pipeline::PipelineFragmentContext>(
...
void FragmentMgr::remove_pipeline_context(std::shared_ptr<pipeline::PipelineFragmentContext> f_context) {
    std::lock_guard<std::mutex> lock(_lock);
