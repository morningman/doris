...
class S3TablesIcebergBackendFactoryTest {
    ...
    void factoryDiscoveredViaServiceLoader() {
        ...
        boolean present = found.stream()
                .anyMatch(f -> f instanceof S3TablesIcebergBackendFactory);
        Assertions.assertTrue(present,
                "ServiceLoader did not find S3TablesIcebergBackendFactory; got: " + found);
    }

    @Test
    void factoryNameMatchesBackendName() {
        S3TablesIcebergBackendFactory f = new S3TablesIcebergBackendFactory();
        Assertions.assertEquals("s3tables", f.name());
