...
public class InternalSchemaInitializer extends Thread {
    ...
    public void run() {
        ...
        modifyTblReplicaCount(database, BuiltinAuditLoaderPlugin.AUDIT_LOG_TABLE);
    }
    ...
    private static CreateTableStmt buildAuditTblStmt() throws UserException {
        TableName tableName = new TableName("",
                FeConstants.INTERNAL_DB_NAME, BuiltinAuditLoaderPlugin.AUDIT_LOG_TABLE);
        ...
        CreateTableStmt createTableStmt = new CreateTableStmt(true, false,
                tableName, InternalSchema.getCopiedSchema(BuiltinAuditLoaderPlugin.AUDIT_LOG_TABLE),
    ...
    private boolean created() {
        ...
        // 3. check audit table
        optionalStatsTbl = db.getTable(BuiltinAuditLoaderPlugin.AUDIT_LOG_TABLE);
        if (!optionalStatsTbl.isPresent()) {
            return false;
        }
        return true;
