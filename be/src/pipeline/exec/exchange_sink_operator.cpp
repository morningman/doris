...
ExchangeSinkOperator::ExchangeSinkOperator(OperatorTemplate* operator_template, vectorized::VDataStreamSender* sink) :
        Operator(operator_template), _part_type(sink->_part_type), _pool(sink->_pool),
        _sink(sink), _current_channel_idx(0), _transfer_large_data_by_brpc(sink->_transfer_large_data_by_brpc) {}
...
Status ExchangeSinkOperator::init(const TDataSink& tsink) {
    ...
    for (auto iter = _sink->_channel_shared_ptrs.begin();
         iter != _sink->_channel_shared_ptrs.end(); iter++) {
...
Status ExchangeSinkOperator::serialize_block(vectorized::Block* src, PBlock* dest, int num_receivers) {
    dest->Clear();
...
Status ExchangeSinkOperator::channel_add_rows(Channels& channels, int num_channels, uint64_t* __restrict channel_ids,
...
ExchangeSinkOperator::Channel::Channel(ExchangeSinkOperator* parent, TUniqueId instance_id,
        const TNetworkAddress& brpc_dest) : _parent(parent), _fragment_instance_id(instance_id),
...
Status ExchangeSinkOperator::Channel::init(RuntimeState *state) {
    ...
    if (!_brpc_stub) {
        std::string msg = fmt::format("ExchangeSinkOperator::Channel::init Get rpc stub failed, dest_addr={}:{}",
...
Status ExchangeSinkOperator::Channel::add_rows(vectorized::Block* block, const std::vector<int>& rows) {
