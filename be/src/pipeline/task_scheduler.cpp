...
//            local_blocked_tasks.splice(local_blocked_tasks.end(), _blocked_tasks);
        ...
        while (iter != local_blocked_tasks.end()) {
            ...
            if (state == PENDING_FINISH || task->fragment_context()->is_canceled()) {
                ...
                LOG(WARNING) << "[Driver] Timeout, query_id=" << print_id(task->query_fragments_context()->query_id)
                             << ", instance_id=" << print_id(task->fragment_context()->get_fragment_id());
...
Status TaskScheduler::start() {
    ...
    for(size_t i = 0; i < cores; ++i) {
        ...
        RETURN_IF_ERROR(_fix_thread_pool->submit_func(std::bind(&TaskScheduler::_do_work, this, i)));
    }
