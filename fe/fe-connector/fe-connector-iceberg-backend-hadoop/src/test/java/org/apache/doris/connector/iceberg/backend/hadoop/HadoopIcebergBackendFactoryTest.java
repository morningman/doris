...
class HadoopIcebergBackendFactoryTest {
    ...
    void factoryDiscoveredViaServiceLoader() {
        ...
        boolean present = found.stream()
                .anyMatch(f -> f instanceof HadoopIcebergBackendFactory);
        Assertions.assertTrue(present,
                "ServiceLoader did not find HadoopIcebergBackendFactory; got: " + found);
    }

    @Test
    void factoryNameMatchesBackendName() {
        HadoopIcebergBackendFactory f = new HadoopIcebergBackendFactory();
        Assertions.assertEquals("hadoop", f.name());
