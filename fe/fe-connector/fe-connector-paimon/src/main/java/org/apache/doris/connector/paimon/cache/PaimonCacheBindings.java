...
/**
 * Paimon connector cache binding declarations. Each binding describes a
 * cache instance owned by the fe-core {@code ConnectorMetaCacheRegistry} on
 * behalf of a single {@code PaimonConnector}. The plugin overrides
 ...
 *
 * <p>Three bindings are declared (mirroring the iceberg shape introduced by
 * M1-10):
 * <ul>
 *   <li>{@link #ENTRY_CATALOG}: keyed by catalog name, value is the paimon
 ...
 *       underlying catalog on eviction (paimon's {@code Catalog} extends
 *       {@link Closeable}).</li>
 *   <li>{@link #ENTRY_TABLE}: keyed by {@link PaimonTableCacheKey}, value is
 *       a loaded paimon {@link Table}. Default capacity 10_000, 1h TTL.
 *       Reacts to {@code CATALOG}, {@code DATABASE}, {@code TABLE}
 *       scopes.</li>
 *   <li>{@link #ENTRY_SNAPSHOTS}: keyed by {@link PaimonTableCacheKey},
 *       value is the materialized snapshot list ({@link Snapshot}).
 *       Capacity 10_000, 5min TTL (snapshot lists drift faster than schema).
 *       Reacts to {@code CATALOG}, {@code DATABASE}, {@code TABLE}.</li>
 * </ul>
 *
 ...
 */
public final class PaimonCacheBindings {

    private static final Logger LOG = LogManager.getLogger(PaimonCacheBindings.class);

    public static final String ENTRY_CATALOG = "paimon.catalog";
    public static final String ENTRY_TABLE = "paimon.table";
    public static final String ENTRY_SNAPSHOTS = "paimon.snapshots";

    private PaimonCacheBindings() {
    }
     ...
     * removal the value is closed (paimon {@code Catalog} extends
     * {@link Closeable}).
     */
    ...
    public static ConnectorMetaCacheBinding<PaimonTableCacheKey, Table> tableBinding(
            CacheLoader<PaimonTableCacheKey, Table> loader) {
        Objects.requireNonNull(loader, "loader");
        ConnectorCacheSpec spec = ConnectorCacheSpec.builder()
                .maxSize(10_000L)
                ...
                .build();
        return ConnectorMetaCacheBinding
                .<PaimonTableCacheKey, Table>builder(
                        ENTRY_TABLE, PaimonTableCacheKey.class, Table.class, loader)
                .defaultSpec(spec)
    ...
    public static ConnectorMetaCacheBinding<PaimonTableCacheKey, List<Snapshot>> snapshotsBinding(
            CacheLoader<PaimonTableCacheKey, List<Snapshot>> loader) {
        Objects.requireNonNull(loader, "loader");
        ConnectorCacheSpec spec = ConnectorCacheSpec.builder()
                .maxSize(10_000L)
        ...
        return ConnectorMetaCacheBinding
                .<PaimonTableCacheKey, List<Snapshot>>builder(
                        ENTRY_SNAPSHOTS, PaimonTableCacheKey.class, valueType, loader)
                .defaultSpec(spec)
                .invalidationStrategy(perTableScope())
                .build();
    }
        ...
        return (key, value, cause) -> {
            if (value instanceof Closeable) {
                try {
                    ...
                    LOG.warn("Failed to close paimon catalog '{}' on cache removal: {}",
                            key, e.toString());
                }
            }
        };
