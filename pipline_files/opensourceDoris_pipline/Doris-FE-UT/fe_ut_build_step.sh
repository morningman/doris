...
bash check_change_file.sh --is_modify_only_invoved_be %teamcity.pullRequest.number% 2>/dev/null
if [[ $? == 0 ]]; then
    exit 0
fi
...
echo "docker run -i --rm --name doris-fe-ut-%build.vcs.number% -e TZ=Asia/Shanghai -v /etc/localtime:/etc/localtime:ro -v /home/work/.m2:/root/.m2 -v /home/work/.npm:/root/.npm -v ${git_storage_path}:/root/git -v %teamcity.build.checkoutDir%:/root/doris apache/doris:build-env-ldb-toolchain-latest /bin/bash -c \"mkdir -p ${git_storage_path} && cp -r /root/git/* ${git_storage_path}/ && export FE_UT_PARALLEL=1 && cd /root/doris && bash run-fe-ut.sh\""
docker run -i --rm --name doris-fe-ut-%build.vcs.number% -e TZ=Asia/Shanghai -v /etc/localtime:/etc/localtime:ro -v /home/work/.m2:/root/.m2 -v /home/work/.npm:/root/.npm -v ${git_storage_path}:/root/git -v %teamcity.build.checkoutDir%:/root/doris apache/doris:build-env-ldb-toolchain-latest /bin/bash -c "mkdir -p ${git_storage_path} && cp -r /root/git/* ${git_storage_path}/ && export FE_UT_PARALLEL=1 && cd /root/doris && bash run-fe-ut.sh"
if [ $? -ne 0 ]; then
