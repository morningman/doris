...
parallel-ssh -h $CLUSTER/fe_master_host -i "cd $CLUSTER_DIR/fe && ulimit -c unlimited -n 1000000 && $EXPORT_ASAN && $EXPORT_UBSAN && ./bin/start_fe.sh --daemon"
parallel-ssh -h $CLUSTER/fe_follower_hosts -i "cd $CLUSTER_DIR/fe && ulimit -c unlimited -n 1000000 && $EXPORT_ASAN && $EXPORT_UBSAN && ./bin/start_fe.sh  --helper $FE_MASTER_HOST:$EDIT_PORT --daemon"
