...
HdfsFileReader::HdfsFileReader(Path path, const std::string& name_node,
    ...
    if (_profile != nullptr) {
        ...
        _hdfs_profile.hedged_read_in_cur_thread =
                ADD_CHILD_COUNTER(_profile, "HedgedReadInCurThread", TUnit::UNIT, hdfs_profile_name);
        _hdfs_profile.hedged_read_wins = ADD_CHILD_COUNTER(
                _profile, "HedgedReadWins", TUnit::UNIT, hdfs_profile_name);
#endif
    }
}
...
Status HdfsFileReader::close() {
    ...
    if (_closed.compare_exchange_strong(expected, true, std::memory_order_acq_rel)) {
        ...
        if (_profile != nullptr) {
            ...
            COUNTER_UPDATE(_hdfs_profile.hedged_read_in_cur_thread, hdfs_hedged_read_statistics->hedgedReadOpsInCurThread); 
            COUNTER_UPDATE(_hdfs_profile.hedged_read_wins, hdfs_hedged_read_statistics->hedgedReadOpsWin); 
