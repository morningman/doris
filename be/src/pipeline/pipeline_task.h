...
public:
    ...
    : _index(index), _pipeline(pipeline), _operators(operators),
      _source(_operators.back()), _root(_operators.front()), _sink(sink), _prepared(false), _opened(false),
      _state(state), _cur_state(NOT_READY), _fragment_context(fragment_context) {
    }
...
//    }
    ...
    bool is_blocking() {
        return has_dependency() || !_source->can_read() || !_sink->can_write();
    }
    ...
    int get_previous_core_id() {
        return _previous_schedule_id;
    }
