...
if [[ "${FE_MODULES}" != '' ]]; then
    ...
    if [[ "${DISABLE_JAVA_CHECK_STYLE}" = "ON" ]]; then
        "${MVN_CMD}" -o package -pl ${FE_MODULES:+${FE_MODULES}} -Dskip.doc=true -DskipTests -Dcheckstyle.skip=true
    else
        "${MVN_CMD}" -o package -pl ${FE_MODULES:+${FE_MODULES}} -Dskip.doc=true -DskipTests
    fi
    cd "${DORIS_HOME}"
fi
