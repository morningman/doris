...
bool PipelineTask::has_dependency() {
    ...
    // fe还未开启查询
    if (!_state->get_query_fragments_ctx()->is_ready_to_execute()) { // TODO pipeline config::s_ready_to_execute
        return true;
    }
...
Status PipelineTask::execute(bool *eos) {
    ...
    *eos = false;
    ...
    while (_source->can_read() && _sink->can_write() && !_fragment_context->is_canceled()) {
        ...
        _block->clear_column_data(_root->operator_template()->exec_node()->row_desc()
                                          .num_materialized_slots());
        auto* block = _block.get();
        LOG(INFO) << "llj log get block " << this;
        RETURN_IF_ERROR(_root->get_block(_state, block, eos));
        if (_block->rows() != 0 || *eos) {
            LOG(INFO) << "llj log get block end " << *eos << " rows:" << _block->rows() << " " << this;
