...
Status JdbcConnector::start_query(RuntimeState* state) {
    return state->exec_env()->jdbc_query_thread_pool()->submit_func(
            [this, state] {
            this->_query_worker(state->batch_size());
            });
}

Status JdbcConnector::_query_worker(int batch_size) {
    JNIEnv* env = nullptr;
    RETURN_IF_ERROR(JniUtil::GetJNIEnv(&env));
    ...
    while (true) {
        ...
        if (has_next != JNI_TRUE) {
            ...
            LOG(INFO) << "yy debug query queue shutdown, queue size: " << _query_queue.get_size();
            break;
        }
        ...
        RETURN_IF_ERROR(JniUtil::GetJniExceptionMsg(env));
        _query_queue.blocking_put(block_obj);
        ++block_num;
    }
    LOG(INFO) << "yy debug total block num: " << block_num;
    return Status::OK();
}
...
Status JdbcConnector::get_next(bool* eos, std::vector<MutableColumnPtr>& columns, Block* block,
    ...
    jobject block_obj;
    bool get = _query_queue.blocking_get(&block_obj);
    if (!get) {
        LOG(INFO) << "yy debug blocking get return false, queue size: " << _query_queue.get_size();
        *eos = true;
        return Status::OK();
    }
    ...
    LOG(INFO) << "yy debug get block rows: " << block->rows();
    return JniUtil::GetJniExceptionMsg(env);
}
