...
/**
 * {@link EventCursor} for the Paimon snapshot-polling event source.
 *
 * <p>Paimon has no global notification log; the cursor instead carries
 * <ul>
 *   <li>a monotonic {@code tickId} bumped on every successful poll —
 *       used for {@link #compareTo(EventCursor)} (paimon's per-table
 *       {@code snapshotId} is monotonic per-table only, not across the
 *       catalog, so a scalar tickId is the only catalog-wide
 *       monotonic position); and</li>
 *   <li>a snapshot of every known {@code (db, table)} → last seen
 *       {@code TableState(snapshotId, schemaId)} so the next poll can
 *       diff against the live catalog without re-emitting historical
 *       events.</li>
 * </ul>
 *
 * <p>The map is stored as a {@link SortedMap} so {@code equals},
 * {@code hashCode} and binary serialization are stable across JVMs.
 */
public final class PaimonEventCursor implements EventCursor {
    ...
    public static final class TableState {
        ...
        public TableState(long snapshotId, long schemaId) {
    ...
    public PaimonEventCursor(long tickId, Map<String, TableState> tables) {
    ...
    /** Build the cursor map key from {@code (db, table)}. */
    ...
    public static String key(Identifier id) {
        ...
        return key(id.getDatabaseName(), id.getObjectName());
    }

    @Override
    public int compareTo(EventCursor other) {
        if (!(other instanceof PaimonEventCursor)) {
            throw new IllegalArgumentException(
                    "cannot compare PaimonEventCursor against " + other.getClass().getName());
        }
        return Long.compare(this.tickId, ((PaimonEventCursor) other).tickId);
    }

    @Override
    public String describe() {
        return "PaimonEventCursor{tickId=" + tickId + ", tables=" + tables.size() + "}";
    }
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof PaimonEventCursor)) {
            return false;
        }
        PaimonEventCursor that = (PaimonEventCursor) o;
