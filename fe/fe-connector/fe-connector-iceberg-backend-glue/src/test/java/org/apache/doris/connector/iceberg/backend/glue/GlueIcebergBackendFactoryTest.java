...
package org.apache.doris.connector.iceberg.backend.glue;
...
class GlueIcebergBackendFactoryTest {
    ...
    void factoryDiscoveredViaServiceLoader() {
        ...
        boolean present = found.stream()
                .anyMatch(f -> f instanceof GlueIcebergBackendFactory);
        Assertions.assertTrue(present,
                "ServiceLoader did not find GlueIcebergBackendFactory; got: " + found);
    }

    @Test
    void factoryNameMatchesBackendName() {
        GlueIcebergBackendFactory f = new GlueIcebergBackendFactory();
        Assertions.assertEquals("glue", f.name());
